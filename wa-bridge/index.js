...
const NEGOCIOS_PATH = process.env.NEGOCIOS_PATH || path.join(__dirname, '..', 'datos', 'negocios.json');
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (statusCode === DisconnectReason.loggedOut || statusCode === DisconnectReason.restartRequired) {
        console.log(`🔴 ${numero} ${statusCode === DisconnectReason.loggedOut ? 'sesión expirada' : 'reintento requerido'}, eliminando auth...`);
